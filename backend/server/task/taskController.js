...
module.exports = {
  ...
  getUsersTasks: async (req, res) => {
    ...
    try {
      ...
      console.log(tasks);
      const tasksWithSupervisor = tasks.map(async task => {
        const supervisorId = await prisma.approverTaskMapping.findMany({
          where: {
            archived: false,
        ...
        return { ...task, supervisorName: supervisor};
      });
      res.status(200).json(tasksWithSupervisor);
    } catch (error) {
